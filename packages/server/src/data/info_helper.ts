...
export class StationInfoHelper {
	...
	constructor({
		...
		this.cleaner = new StationCleaner({ config })
		this.oauth = oauth
		this.config = config
	}
	...
	private async fetchStations({
		...
		while (true) {
			...
			const result = await authenticatedPatientFetch(
				this.oauth,
				`${baseUrl(this.config)}/v1/pfs?${params.toString()}`,
				{
					headers: {
	...
	/**
	 * Fetch and preprocess the complete station dataset. Cleaning and
	 * persistence are handled by the caller so the backfill can run in
	 * resumable chunks instead of one all-or-nothing pass.
	 */
	public async fetchAllStationsPreprocessed(): Promise<PreprocessedStation[]> {
