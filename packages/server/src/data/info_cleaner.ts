...
import { backoffDelayMs } from '../util/concurrency'

/**
 * The system prompts are loaded from disk relative to this module. Wrangler
 * used to turn `.md` imports into raw text; Bun and Vite interpret such
 * imports differently (rendered HTML / asset URLs), so an explicit
 * filesystem read is the only loader-independent way to keep the exact
 * Markdown that the paid cleaning passes depend on.
 */
...
const namesPrompt = loadPrompt('../prompts/names.md')
const addressPrompt = loadPrompt('../prompts/addresses.md')
...
export class StationCleaner {
	...
	constructor({ config }: { config: Pick<ServerConfig, 'openRouterApiKey'> }) {
		this.apiKey = config.openRouterApiKey
	}

	/**
	 * The OpenRouter client is created lazily so a read-only deployment can
	 * run without a key; cleaning fails with an actionable error instead of
	 * an opaque HTTP 401 from a half-configured client.
	 */
	private get openrouterClient(): OpenRouter {
		if (!this.client) {
			if (!this.apiKey) {
				throw new Error(
					'OPENROUTER_API_KEY is not set, but station cleaning requires it. ' +
	...
	private async callLLMWithRetry<TInput, TOutput extends { id: number }>({
		...
		for (let attempt = 1; attempt <= MAX_LLM_RETRIES; attempt++) {
			try {
				...
				// Exponential backoff with full jitter so a struggling upstream
				// is not immediately re-hit by every concurrent batch.
